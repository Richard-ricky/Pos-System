...
// Admin client — reused for both privileged ops AND JWT verification.
// Using the service role key means we can call getUser() without needing
// the user's JWT forwarded in global headers (which Supabase's proxy can strip).
 ...
 * Tries Authorization header (with/without "Bearer "), then X-Auth-Token,
 * then a ?token= query param as a last resort.
 */
...
// Auth middleware — validates JWT via the admin client so header-stripping
// by the Supabase edge proxy never causes a false 401

const authMiddleware: MiddlewareHandler<AppEnv> = async (
  c: Context<AppEnv>,
  next,
) => {
  console.log("Headers received:", JSON.stringify(Object.fromEntries(c.req.raw.headers)));
  ...
  try {
    // getUser() with the admin client verifies the JWT signature server-side
    // without relying on header forwarding.
    const { data, error } = await getAdminClient().auth.getUser(token);

    if (error || !data.user) {
      console.log("Auth: invalid token —", error?.message ?? "no user");
    ...
    console.log("Auth middleware exception:", msg);
...
app.post("/make-server-45351b4f/auth/signup", async (c) => {
  try {
    ...
    console.log("Signup exception:", msg);
...
app.get("/make-server-45351b4f/auth/profile", authMiddleware, async (c) => {
  try {
    ...
    const msg = error instanceof Error ? error.message : "Unknown error";
    console.log("Profile fetch error:", msg);
    return c.json({ error: msg }, 500);
  }
});
...
app.delete("/make-server-45351b4f/products/:id", authMiddleware, async (c) => {
  try {
    ...
    if (profile?.role !== "admin") {
      return c.json({ error: "Only admins can delete products" }, 403);
    }
      ...
      if (customer) {
        const updated = {
          ...
          loyaltyPoints:
            ...
            Math.floor((transactionData.amount as number) / 10),
          lastTransaction: new Date().toISOString(),
        };
        await kv.set(transactionData.customerId as string, updated);
      }
    }
...
app.post("/make-server-45351b4f/payments/verify", authMiddleware, async (c) => {
  try {
    ...
    if (!result.status) return c.json({ error: result.message || "Payment verification failed" }, 400);
...
app.post("/make-server-45351b4f/wallet/fund", authMiddleware, async (c) => {
  try {
    ...
    const updatedWallet = {
      ...wallet,
      balance: (Number(wallet.balance) || 0) + verifyResult.data.amount / 100,
    };
    ...
    return c.json({ success: true, wallet: updatedWallet, transaction });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    return c.json({ error: msg }, 500);
  }
...
app.post("/make-server-45351b4f/accounts/link-card", authMiddleware, async (c) => {
  try {
    ...
    const card = { id: cardId, userId: user.id, ...cardData, createdAt: new Date().toISOString() };
    await kv.set(cardId, card);
    return c.json({ card });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    return c.json({ error: msg }, 500);
...
app.post("/make-server-45351b4f/accounts/link-momo", authMiddleware, async (c) => {
  try {
    ...
    const momo = { id: momoId, userId: user.id, ...momoData, createdAt: new Date().toISOString() };
    await kv.set(momoId, momo);
    return c.json({ momo });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    return c.json({ error: msg }, 500);
...
app.get("/make-server-45351b4f/ml/recommendations", authMiddleware, async (c) => {
  try {
    ...
    const userBehavior = {
      ...
      averageAmount:
        userTransactions.reduce((sum, t) => sum + Number(t.amount ?? 0), 0) /
    ...
    const recommendations = ProductRecommendationEngine.getPersonalizedRecommendations(productMetrics, userBehavior, 10);
    const promotedProducts = ProductRecommendationEngine.getPromotedProducts(productMetrics, 10);
    const lowStockPromotions = ProductRecommendationEngine.getLowStockPromotions(productMetrics, 10);

    return c.json({ personalized: recommendations, promoted: promotedProducts, lowStock: lowStockPromotions });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    return c.json({ error: msg }, 500);
  }
  ...
  try {
    ...
    if (twilioAccountSid && twilioAuthToken) {
      try {
        ...
        if (!response.ok) throw new Error(data.message || "Failed to send SMS via Twilio");
        return c.json({ success: true, provider: "twilio", sid: data.sid, messageId, status: "sent" });
      } catch (error: unknown) {
        const msg = error instanceof Error ? error.message : "Unknown error";
        console.log("Twilio SMS error:", msg);
      }
    }
    ...
    return c.json({
      ...
      messageId,
      status: "sent",
      message: "SMS sent in development mode (configure Twilio for production)",
    });
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : "Unknown error";
    return c.json({ error: msg }, 500);
...
app.get("/make-server-45351b4f/sms/:messageId", async (c) => {
  try {
    ...
    const smsData = await kv.get(`sms:${messageId}`);
